...
def get_path(prefix, path_elem, path_type):
    exit_not_found=None
    ...
    if exit_not_found is True:
        if not os.path.exists(path_elem):
            msg.error("Path not found '{}'".format(path_elem), prefix=prefix, exit=1)
        if path_type == "dir":
            if not os.path.isdir(path_elem):
                msg.error("Path is not a directory '{}'".format(path_elem), prefix=prefix, exit=1)
        elif path_type == "file":
            if not os.path.isfile(path_elem):
                msg.error("Path is not a file '{}'".format(path_elem), prefix=prefix, exit=1)
